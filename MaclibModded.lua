...
--// Services
local TweenService = MacLibModded.GetService("TweenService")
local RunService = MacLibModded.GetService("RunService")
local HttpService = MacLibModded.GetService("HttpService")
local ContentProvider = MacLibModded.GetService("ContentProvider")
local UserInputService = MacLibModded.GetService("UserInputService")
local Lighting = MacLibModded.GetService("Lighting")
local Players = MacLibModded.GetService("Players")
...
local function GetGui()
	...
	local parent = RunService:IsStudio() 
		...
		or (cloneref and cloneref(MacLibModded.GetService("CoreGui")) or MacLibModded.GetService("CoreGui"))
...
--// Library Functions
function MacLibModded:Window(Settings)
		...
		function SectionFunctions:Tab(Settings)
			...
			function TabFunctions:Section(Settings)
				...
				function SectionFunctions:Button(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = ButtonFunctions
					end
				...
				function SectionFunctions:Toggle(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = ToggleFunctions
					end
				...
				function SectionFunctions:Slider(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = SliderFunctions
					end
				...
				function SectionFunctions:Input(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = InputFunctions
					end
				...
				function SectionFunctions:Keybind(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = KeybindFunctions
					end
				...
				function SectionFunctions:Dropdown(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = DropdownFunctions
					end
				...
				function SectionFunctions:Colorpicker(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = ColorpickerFunctions
					end
				...
				function SectionFunctions:Header(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = HeaderFunctions
					end
				...
				function SectionFunctions:Label(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = LabelFunctions
					end
				...
				function SectionFunctions:SubLabel(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = SubLabelFunctions
					end
				...
				function SectionFunctions:Paragraph(Settings, Flag)
					...
					if Flag then
						MacLibModded.Options[Flag] = ParagraphFunctions
					end
			...
			function TabFunctions:InsertConfigSection(Side)
				...
				local configSelection = configSection:Dropdown({
					...
					Multi = false,
					Required = false,
					Options = MacLibModded:RefreshConfigList(),
				...
				configSection:Button({
					...
					Callback = function()
						...
						local success, returned = MacLibModded:SaveConfig(inputPath)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
						...
						configSelection:InsertOptions(MacLibModded:RefreshConfigList())
					end,
				})
				...
				configSection:Button({
					Name = "Load Config",
					Callback = function()
						local success, returned = MacLibModded:LoadConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						local success, returned = MacLibModded:SaveConfig(configSelection.Value)
						if not success then
							WindowFunctions:Notify({
								Title = "Interface",
				...
				configSection:Button({
					...
					Callback = function()
						...
						writefile(MacLibModded.Folder .. "/settings/autoload.txt", name)
				...
				if isfile(MacLibModded.Folder .. "/settings/autoload.txt") then
					local name = readfile(MacLibModded.Folder .. "/settings/autoload.txt")
					autoloadLabel:UpdateName("Autoload config: " .. name)
				end
			end
	...
	local ClassParser = {
		["Toggle"] = {
			...
			Load = function(Flag, data)
				if MacLibModded.Options[Flag] and data.state then
					MacLibModded.Options[Flag]:UpdateState(data.state)
				end
			end
		},
		["Slider"] = {
			...
			Load = function(Flag, data)
				if MacLibModded.Options[Flag] and data.value then
					MacLibModded.Options[Flag]:UpdateValue(data.value)
				end
			end
		},
		["Input"] = {
			...
			Load = function(Flag, data)
				if MacLibModded.Options[Flag] and data.text and type(data.text) == "string" then
					MacLibModded.Options[Flag]:UpdateText(data.text)
				end
			end
		},
		["Keybind"] = {
			...
			Load = function(Flag, data)
				if MacLibModded.Options[Flag] and data.bind then
					MacLibModded.Options[Flag]:Bind(Enum.KeyCode[data.bind])
				end
			end
		},
		["Dropdown"] = {
			...
			Load = function(Flag, data)
				if MacLibModded.Options[Flag] and data.value then
					MacLibModded.Options[Flag]:UpdateSelection(data.value)
				end
			end
		},
		["Colorpicker"] = {
			...
			Load = function(Flag, data)
				...
				if MacLibModded.Options[Flag] and data.color then
					MacLibModded.Options[Flag]:SetColor(HexToColor3(data.color)) 
					if data.alpha then
						MacLibModded.Options[Flag]:SetAlpha(data.alpha)
					end
				end
			end
		}
	...
	function MacLibModded:LoadAutoLoadConfig()
		if isStudio or not (isfile and readfile) then return "Config system unavailable." end

		if isfile(MacLibModded.Folder .. "/settings/autoload.txt") then
			local name = readfile(MacLibModded.Folder .. "/settings/autoload.txt")

			local suc, err = MacLibModded:LoadConfig(name)
			if not suc then
				WindowFunctions:Notify({
					Title = "Interface",
	...
	function MacLibModded:SetFolder(Folder)
		if isStudio then return "Config system unavailable." end

		MacLibModded.Folder = Folder;
		BuildFolderTree()
	end

	function MacLibModded:SaveConfig(Path)
		...
		local fullPath = MacLibModded.Folder .. "/settings/" .. Path .. ".json"

		local data = {
			objects = {}
		}

		for flag, option in next, MacLibModded.Options do
	...
	function MacLibModded:LoadConfig(Path)
		if isStudio or not (isfile and readfile) then return "Config system unavailable." end
		...
		local file = MacLibModded.Folder .. "/settings/" .. Path .. ".json"
	...
	function MacLibModded:RefreshConfigList()
		if isStudio or not (isfolder and listfiles) then return "Config system unavailable." end

		local list = (isfolder(MacLibModded.Folder) and isfolder(MacLibModded.Folder .. "/settings")) and listfiles(MacLibModded.Folder .. "/settings") or {}
	...
	-- //  Floating Button (FB) - toggles MacLibModded window visibility
	function WindowFunctions:FB(Settings)
		local FBFunctions = { Settings = Settings }

		-- Parse scale size from Settings.Size (e.g. "0.48", "0.48")
		local sizeX = tonumber(Settings.Size and Settings.Size[1]) or 0.08
		local sizeY = tonumber(Settings.Size and Settings.Size[2]) or 0.08

		-- Dedicated ScreenGui for the floating button
		local fbGui = Instance.new("ScreenGui")
		fbGui.Name = "MacLibModded_FloatingButton"
		fbGui.ScreenInsets = Enum.ScreenInsets.None
		fbGui.ResetOnSpawn = false
		fbGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
		fbGui.DisplayOrder = 2147483646

		local fbGuiParent = RunService:IsStudio()
			and LocalPlayer:FindFirstChild("PlayerGui")
			or (gethui and gethui())
			or (cloneref and cloneref(MacLibModded.GetService("CoreGui")) or MacLibModded.GetService("CoreGui"))
		fbGui.Parent = fbGuiParent
		...
		fbFrame.Size = UDim2.fromScale(sizeX, sizeY)
		fbFrame.Position = UDim2.fromScale(0.02, 0.44)
		fbFrame.Parent = fbGui

		-- Main image button
		local fbButton = Instance.new("ImageButton")
		fbButton.Name = "FBButton"
		fbButton.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
		fbButton.BackgroundTransparency = 0.08
		fbButton.BorderSizePixel = 0
		fbButton.Size = UDim2.fromScale(1, 1)
		fbButton.Image = Settings.Image or ""
		fbButton.ImageTransparency = 0
		fbButton.AutoButtonColor = false
		...
		fbStroke.Transparency = 0.82
		fbStroke.Parent = fbButton

		-- Title label at bottom of button
		if Settings.Title then
			local fbTitle = Instance.new("TextLabel")
			fbTitle.Name = "FBTitle"
			fbTitle.FontFace = Font.new(
				assets.interFont,
				Enum.FontWeight.SemiBold,
				Enum.FontStyle.Normal
			)
			fbTitle.Text = Settings.Title
			fbTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
			fbTitle.TextSize = 10
			fbTitle.TextTransparency = 0.15
			fbTitle.TextWrapped = true
			fbTitle.BackgroundTransparency = 1
			fbTitle.BorderSizePixel = 0
			fbTitle.Size = UDim2.new(1, 0, 0.28, 0)
			fbTitle.AnchorPoint = Vector2.new(0, 1)
			fbTitle.Position = UDim2.fromScale(0, 1)
			fbTitle.Parent = fbButton
		end

		-- Hover glow effect
		fbButton.MouseEnter:Connect(function()
			Tween(fbButton, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
				BackgroundTransparency = 0,
				ImageTransparency = 0
			}):Play()
			Tween(fbStroke, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
				Transparency = 0.6
			}):Play()
		end)
		fbButton.MouseLeave:Connect(function()
			Tween(fbButton, TweenInfo.new(0.15, Enum.EasingStyle.Sine), {
		...
		if Settings.Draggable then
			...
			fbButton.InputBegan:Connect(function(input)
				if input.UserInputType == Enum.UserInputType.MouseButton1
					or input.UserInputType == Enum.UserInputType.Touch then
					dragging = true
					startMousePos = input.Position
					startFramePos = fbFrame.Position
			...
			fbButton.InputChanged:Connect(function(input)
		...
		-- Click to toggle MacLibModded window (distinguishes click from drag)
		...
		fbButton.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1
				or input.UserInputType == Enum.UserInputType.Touch then
				local moved = (Vector2.new(input.Position.X, input.Position.Y) - clickStartPos).Magnitude
				if moved < 6 then
		...
		-- FBFunctions API
		function FBFunctions:SetVisibility(State)
			fbFrame.Visible = State
		end
		function FBFunctions:GetVisibility()
			return fbFrame.Visible
		end
		function FBFunctions:Destroy()
			fbGui:Destroy()
		end
...
function MacLibModded:Demo()
	local Window = MacLibModded:Window({
		Title = "Maclib Demo",
	...
	tabs.Main:Select()
	MacLibModded:LoadAutoLoadConfig()
end
